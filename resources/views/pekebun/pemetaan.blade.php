...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="flex items-start">
            ...
            <div>
              ...
              <p class="text-sm text-green-700 mt-1">
                ...
                3. Klik titik awal untuk menutup area<br>
                4. Area hijau yang terbentuk adalah lahan Anda, kemudian klik <strong>Simpan Peta
                  Lahan</strong>
              </p>
            </div>
          </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-slate-100 overflow-hidden">
        <div
          ...
          <div>
            <h2 class="text-base font-semibold text-slate-900 flex items-center gap-2">
              <span
                class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 text-xs">
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-1">
              ...
              batas lahan Anda.
              Tutup kembali ke titik awal untuk menyelesaikan gambar.
            </p>
          </div>
        ...
        <div class="px-5 sm:px-5 pb-5">
          <div class="relative">
            ...
            <!-- Legenda -->
            <div
              class="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm shadow-md rounded-lg px-3 py-2 text-[11px] text-slate-600 space-y-1 border border-slate-100 z-10">
          ...
          <form action="{{ route('pekebun.pemetaan.simpan', $kebun->id) }}" method="POST"
            class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between" id="pemetaan-form">
            @csrf
            ...
            <div class="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
              <div class="inline-flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-1.5 border border-slate-200">
                <span class="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">Centroid</span>
                <span class="font-mono text-[11px] sm:text-xs text-emerald-700">
                  Lat:
              ...
              <p class="text-[11px] text-slate-400">
                * Centroid adalah titik tengah perkiraan dari area lahan yang Anda gambar.
              </p>
            </div>
      ...
      <div class="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg border border-slate-100 px-4 py-3.5 shadow-sm">
          ...
          @else
            <p
              class="inline-flex items-center gap-1.5 rounded-full bg-slate-50 px-3 py-1 text-xs font-medium text-slate-500">
  ...
  <script>
    const defaultLat = 0.8833; // kira-kira Rokan Hulu
    const defaultLng = 100.4833;

    const allKebun = @json($allKebun ?? []);
    const currentKebunId = @json($kebun->id);

    // Feature kebun lain (dipakai untuk cek overlap)
    ...
    function initMap() {
      ...
      (allKebun || []).forEach(function (item) {
        ...
        polyLayer.eachLayer(function (layer) {
          ...
          if (isCurrent) {
            ...
            persistPolygon(layer); // set centroid + hidden input
          } else {
          ...
          const iconHtml = `
            <div class="kebun-marker-pin ${isCurrent
              ? 'kebun-marker-pin--current'
          ...
          const marker = L.marker([markerLat, markerLng], { icon }).addTo(mapInstance);
          ...
          const tooltipHtml = `
            <div class="text-[11px]">
              <div class="font-semibold text-slate-800 mb-0.5">${item.nama_kebun || 'Kebun Tanpa Nama'
            }</div>
              <div class="text-slate-500">Pemilik: <span class="font-medium">${item.pemilik || '-'
            }</span></div>
      ...
      if (totalBounds) {
        mapInstance.fitBounds(totalBounds, { padding: [24, 24] });
      }
    ...
    // Cek apakah polygon baru bertumpuk dengan kebun lain
    function intersectsOtherKebun(newGeometry) {
      ...
      if (typeof turf === 'undefined' || !turf.booleanIntersects) {
        console.warn(
          'Turf.js belum dimuat, cek overlap tidak dijalankan. Pastikan CDN Turf sudah ditambahkan.'
        );
        return false;
      }
      ...
      for (const other of otherPolygons) {
        // booleanIntersects: true jika area saling beririsan (overlap atau nempel)
    ...
    function persistPolygon(layer) {
      ...
      // 🔴 Cek overlap dengan kebun lain
      if (intersectsOtherKebun(geojson.geometry)) {
        alert(
          'Area lahan yang Anda gambar bertumpuk dengan lahan pekebun lain. Silakan sesuaikan batasnya.'
        );
